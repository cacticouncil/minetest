...
private:
	...
	// set_node(pos, node)
	// pos = {x=num, y=num, z=num}
	static int l_native_set_node(lua_State *L);

	// bulk_set_node([pos1, pos2, ...], node)
	// pos = {x=num, y=num, z=num}
	static int l_native_bulk_set_node(lua_State *L);

	static int l_native_add_node(lua_State *L);

	// remove_node(pos)
	// pos = {x=num, y=num, z=num}
	static int l_native_remove_node(lua_State *L);

	// swap_node(pos, node)
	// pos = {x=num, y=num, z=num}
	static int l_native_swap_node(lua_State *L);

	// get_node(pos)
	// pos = {x=num, y=num, z=num}
	static int l_native_get_node(lua_State *L);
	...
	// pos = {x=num, y=num, z=num}
	// timeofday: nil = current time, 0 = night, 0.5 = day
	static int l_native_get_node_light(lua_State *L);

	// get_natural_light(pos, timeofday)
	// pos = {x=num, y=num, z=num}
	// timeofday: nil = current time, 0 = night, 0.5 = day
	static int l_native_get_natural_light(lua_State *L);

	// place_node(pos, node)
	// pos = {x=num, y=num, z=num}
	static int l_native_place_node(lua_State *L);

	// dig_node(pos)
	// pos = {x=num, y=num, z=num}
	static int l_native_dig_node(lua_State *L);

	// punch_node(pos)
	// pos = {x=num, y=num, z=num}
	static int l_native_punch_node(lua_State *L);

	// get_node_max_level(pos)
	// pos = {x=num, y=num, z=num}
	static int l_native_get_node_max_level(lua_State *L);

	// get_node_level(pos)
	// pos = {x=num, y=num, z=num}
	static int l_native_get_node_level(lua_State *L);

	// set_node_level(pos)
	// pos = {x=num, y=num, z=num}
	static int l_native_set_node_level(lua_State *L);

	// add_node_level(pos)
	// pos = {x=num, y=num, z=num}
	static int l_native_add_node_level(lua_State *L);

	// find_nodes_with_meta(pos1, pos2)
	static int l_native_find_nodes_with_meta(lua_State *L);

	// get_meta(pos)
	static int l_native_get_meta(lua_State *L);

	// get_node_timer(pos)
	static int l_native_get_node_timer(lua_State *L);

	// add_entity(pos, entityname) -> ObjectRef or nil
	// pos = {x=num, y=num, z=num}
	static int l_native_add_entity(lua_State *L);
	...
	// get_connected_players()
	static int l_native_get_connected_players(lua_State *L);

	// get_player_by_name(name)
	static int l_native_get_player_by_name(lua_State *L);

	// get_objects_inside_radius(pos, radius)
	static int l_native_get_objects_inside_radius(lua_State *L);

	// get_objects_in_area(pos, minp, maxp)
	static int l_native_get_objects_in_area(lua_State *L);

	// set_timeofday(val)
	// val = 0...1
	static int l_native_set_timeofday(lua_State *L);

	// get_timeofday() -> 0...1
	static int l_native_get_timeofday(lua_State *L);

	// get_gametime()
	static int l_native_get_gametime(lua_State *L);

	// get_day_count() -> int
	static int l_native_get_day_count(lua_State *L);

	// find_node_near(pos, radius, nodenames, search_center) -> pos or nil
	// nodenames: eg. {"ignore", "group:tree"} or "default:dirt"
	static int l_native_find_node_near(lua_State *L);

	// find_nodes_in_area(minp, maxp, nodenames) -> list of positions
	// nodenames: eg. {"ignore", "group:tree"} or "default:dirt"
	static int l_native_find_nodes_in_area(lua_State *L);

	// find_surface_nodes_in_area(minp, maxp, nodenames) -> list of positions
	// nodenames: eg. {"ignore", "group:tree"} or "default:dirt"
	static int l_native_find_nodes_in_area_under_air(lua_State *L);

	// fix_light(p1, p2) -> true/false
	static int l_native_fix_light(lua_State *L);

	// load_area(p1)
	static int l_native_load_area(lua_State *L);

	// emerge_area(p1, p2)
	static int l_native_emerge_area(lua_State *L);

	// delete_area(p1, p2) -> true/false
	static int l_native_delete_area(lua_State *L);

	// get_perlin(seeddiff, octaves, persistence, scale)
	// returns world-specific PerlinNoise
	static int l_native_get_perlin(lua_State *L);

	// get_perlin_map(noiseparams, size)
	// returns world-specific PerlinNoiseMap
	static int l_native_get_perlin_map(lua_State *L);

	// get_voxel_manip()
	// returns world-specific voxel manipulator
	static int l_native_get_voxel_manip(lua_State *L);

	// clear_objects()
	// clear all objects in the environment
	static int l_native_clear_objects(lua_State *L);

	// spawn_tree(pos, treedef)
	static int l_native_spawn_tree(lua_State *L);

	// line_of_sight(pos1, pos2) -> true/false
	static int l_native_line_of_sight(lua_State *L);

	// raycast(pos1, pos2, objects, liquids) -> Raycast
	static int l_native_raycast(lua_State *L);

	// find_path(pos1, pos2, searchdistance,
	//     max_jump, max_drop, algorithm) -> table containing path
	static int l_native_find_path(lua_State *L);

	// transforming_liquid_add(pos)
	static int l_native_transforming_liquid_add(lua_State *L);

	// forceload_block(blockpos)
	// forceloads a block
	static int l_native_forceload_block(lua_State *L);

	// forceload_free_block(blockpos)
	// stops forceloading a position
	static int l_native_forceload_free_block(lua_State *L);

	// Get a string translated server side
	static int l_native_get_translated_string(lua_State *L);
